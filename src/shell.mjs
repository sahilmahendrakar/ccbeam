...
/**
 * Shell integration: make `claude` mean `ccteleport`.
 *
...
export const START = "# >>> ccteleport >>>";
export const END = "# <<< ccteleport <<<";
...
export function blockFor(shell) {
  const note = "# Makes `claude` teleport-capable. Remove with: ccteleport uninstall-shell";
  if (shell === "fish") {
    return [START, note, "function claude", "    command ccteleport $argv", "end", END].join("\n");
  }
  // `command` so this function never calls itself.
  return [START, note, 'claude() { command ccteleport "$@"; }', END].join("\n");
}
